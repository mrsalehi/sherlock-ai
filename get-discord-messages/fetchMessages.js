...
const CHANNEL_ID = "1072944049788555314";
// https://discord.com/channels/1038097195422978059/1072944049788555314
let messages = [];
...
async function readThread(messageId) {
  const url = `${DISCORD_API_URL}/channels/${messageId}/messages?limit=100&around=${messageId}`;
  console.log(url);
  const response = await fetch(url, {
      "headers": {
        "accept": "*/*",
  ...
  const json = await response.json();
  messages.push(json.map(message => { 
    return `${message.author.username}#${message.author.discriminator}(${message.timestamp}): ${message.content}}`
  }).join('\n\n'));
}

// https://discord.com/channels/1038097195422978059/1072944049788555314/1116989141549334619
async function main() {
    // await getMessages(CHANNEL_ID, 0);
    // fs.writeFileSync("messages2.json", JSON.stringify(messages));
    // const response = await fetch("https://discord.com/api/v9/channels/1116989141549334619/messages?around=1116989141549334619", {
    // "headers": {
    //     "accept": "*/*",
    //     "accept-language": "en-US,en;q=0.9",
    //     "authorization": "MTExNzE2Nzc0NDIxNTk0MTE2MA.G38ry7.Pl2xMV-bLH4_L-DRUiuJWxoeRbA573CJtD8Wvw",
    ...
    await readThread("1116985133338214512");
    await readThread("1116982706576838748");
    await readThread("1116838479876325440");
    await readThread("1116862277954588682");
    await readThread("1116989141549334619"); // << markee
    fs.writeFileSync("messages3.json", JSON.stringify(messages));
    messages.forEach(message =>  console.log(message.split(' ').length))
}
main()
